...
export class NotesreportFormComponent implements OnInit {
  ...
  constructor(
    ...
    private sendNotesreportService: SendNotesreportService
  ) {
  ...
  onLevelChange() {
    ...
    this.sendNotesreportService.getAllCoef(levelValue).subscribe((coefs) => {
      this.coefs.clear();
      this.coefs = new Map<string, number>(
        Object.entries(coefs) as [string, number][]
      );
      this.subjects = Array.from(this.coefs.keys());
      this.subjects.forEach((subject) => {
        notesGroup.addControl(
          subject,
  ...
  checkAverage(notesReport: any) {
    ...
    this.subjects.forEach((subject) => {
      if (notesReport['notes'][subject] !== null) {
        totalNotes +=
          notesReport['notes'][subject] * Number(this.coefs.get(subject));
        totalCoefficients += Number(this.coefs.get(subject));
      }
    });
    const average = totalNotes / totalCoefficients;
    this.average = average;
    if (this.average < 12.0 && this.average >= 10.0) {
      this.mention = 'Passable';
    } else if (this.average < 14.0 && this.average >= 12.0) {
      this.mention = 'Assez Bien';
    } else if (this.average < 16.0 && this.average >= 14.0) {
      this.mention = 'Bien';
    } else if (this.average < 18.0 && this.average >= 16.0) {
      this.mention = 'Tres Bien';
    } else if (this.average >= 18.0) {
  ...
  onSubmit() {
    ...
    if (this.notesForm.valid && this.averageValid) {
      ...
      this.sendNotesreportService
        .saveNotes(notesReportForm)
        .subscribe((response) => {
